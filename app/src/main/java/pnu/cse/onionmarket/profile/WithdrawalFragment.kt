    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.withdrawalButton.setOnClickListener {
            ...
            if(password != passwordConfirm) {
                Toast.makeText(context,"두개의 비밀번호가 일치하지않습니다.", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }

            val user = Firebase.auth.currentUser
            val userId = user?.uid
            val userCredential = EmailAuthProvider.getCredential(user?.email.toString(), password)
            ...
            user?.reauthenticate(userCredential)
                ...
                ?.addOnFailureListener {
                    Toast.makeText(context,"계정의 비밀번호가 일치하지않습니다.", Toast.LENGTH_SHORT).show()
                }
        }
    }
}